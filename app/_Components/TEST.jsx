...
const Calendar = ({  events,labelColor }) => {
      ...
      <>
         ...
         <div className="timetable" style={{
            ...
            {daysArray.map((day, dayIndex) => {
               if (day.type === 'empty') {
                  return <div key={day.key} style={{ width: "100%" }} className="day empty"></div>;
               } else {
                  let bgColor;
                  ...
                  events.forEach((event, index) => {
                     ...
                     if (isDateInRange(new Date(currentDate.getFullYear(), currentDate.getMonth(), day.value), event.StartDate, event.EndDate)) {
                        if (new Date(event?.StartDate).toDateString() === new Date(event?.EndDate).toDateString()) {
                           ...
                           bgColor = event?.color||coolBrightColors[index]
                        }
                     }
                  });
                  ...
                  return (
                     ...
                     <div key={day.key} onDoubleClick={() => {
                           ...
                           return (
                              <>
                                 <div key={index} className="multi-day-bar" style={{
                                    gridColumn: `${gridColumnStart} / ${gridColumnEnd}`, gridRow: `${Math.floor(dayIndex / 7) + 2}`, height: "1rem", backgroundColor: event?.color || coolBrightColors[index % 25], borderRadius: "15px",
                                 ...
                                 {multiDayEventCount > 2 && (
                                    <div className="multi-day-count" style={{ position: "absolute", top: "4.5rem", left: "0.5rem", borderRadius: "5px", padding: "0.2rem 0.5rem", fontSize: "0.8rem", overflow: "auto", color: labelColor||"black" }}>
                        ...
                        {events
                           .filter(item => formatDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), day.value)) === item.date)
                           .slice(0, 3) 
                           .map((item, index) => (
                              <div
                                 key={index}
                                 ...
                                 style={{
                                    height: '1rem',
                                    width: '1rem',
                                    backgroundColor: item?.color || coolBrightColors[index],
                                    borderRadius: '50%',
                                    marginBottom: '0.2rem',
                                 }}
                              />
                        ...
                        {events.filter(item => formatDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), day.value)) === item.date).length > 4 && (

                              <h1 style={{ zIndex: "100", fontSize: "1rem", color: labelColor||"black",margin:"0 0.2rem" }}>+{events.filter(item => formatDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), day.value)) === item.date).length - 3}</h1>
                  
                        )}
                        {day.value === currentDay && currentMonth === currentDate.getMonth() && currentYear === currentDate.getFullYear() && (
                           <h1 style={{ zIndex: "100", fontSize: "1rem", color: labelColor || "black" }}>Today</h1>
                        )}
                     </div>
                     </div>
                  );
